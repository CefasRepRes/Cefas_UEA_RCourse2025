...
It is my preference to work in `terra` for vectors as well as rasters. This is simply because it is easier to work in one package where you know that functions are designed to work with both datatypes, and you wont face issues with converting between.

The following is from rspatial.org by Robert Hijmans, with modifications.

# Vector data manipulation

Read in the lux shapefile from yesterday and call it p.

```{r, vec1}
library(terra)
f <- system.file("ex/lux.shp", package = "terra")

# turn f into spatvector p

```

As a quick reminder of yesterdays work, plot `p` with unique colours for '`NAME_2`'
...
You can also extract the geometry as a matrix (this is rarely needed).
...
dfr <- data.frame(District = p$NAME_1, Canton = p$NAME_2, ValueCol = round(runif(length(p), 100, 1000)))

# add attributes to a spatvector
p2 <- p
values(p2) <- dfr
head(p2)

# merge attributes to a spatvector
...
```{r}
# subset p to the first geometry
a <- p[1]
plot(a)

# subset p to all but the first geometry
aa <- p[-1]
plot(aa)
...
`rbind` (`c` for older versions of terra) allows you to append `SpatVect` objects with different attribute names, unlike the standard `rbind` for `data.frame`s.
...
```{r, aggnodis}
pag <- aggregate(p, dissolve = FALSE)
pag
plot(pag, col = "light gray")
```

This is a structure that is similar to what you may get for an archipelago: multiple polygons represented as one entity (one row). Use `disagg` to split these up into their parts and save it as 'pd'. Try this yourself - type `??terra::disagg` into the console to find out what arguments you need to put (if any!).
...
Erase a part of a SpatVector by creating a spatExtent (bounding box) half it's extent. Run the below code in an R script window.

```{r raser}
# This is the extent of p
bb <- ext(p)
plot(bb)

# examine the extent and create a new spatvector which is half the width
ext(p) # SpatExtent : 5.74414015, 6.52825212, 49.44780731, 50.18162155 (xmin, xmax, ymin, ymax) 

# split in half by changing the bounding x and y points
bb <- ext(6.15, 6.528, 49.447, 50.18) # x, x, y, y
...
You can `intersect` or `crop` with a SpatExtent. The difference between `intersect` and `crop` is that with `crop` the geometry of the second argument is not added to the output.
...
```{r}
# subset p into two different spatvectors
p1 <- p[c(1, 4, 5)]
plot(p1)
...
Create a spatraster 'x' from scratch:

```{r, raster-1a}
library(terra)
# Create a SpatRaster with the default parameters (use '?terra::rast()' if unsure)
...
While we can create a `SpatRaster` from scratch, it is more common to do so from a file.
...
Several 'high level' functions have been implemented for `SpatRaster` objects. 'High level' functions refer to functions that you would normally find in a computer program that supports the analysis of raster data. All these functions work for raster datasets that cannot be loaded into memory. See the help files for more detailed descriptions of each function.
...
1.  Install the rnatural earth package and create two sf maps of Germany (shown below). Convert these to SpatVectors

2.  Subset the second map to the first 5 geometries, label it map3

3.  Use the 'geoData' package to download a raster of temperature (shown below). How many layers does it have?

4.  Plot the mean temperature over all the layers

5.  Plot the average temperature but only for the areas in map3

6.  What is the average minimum temperature, for each month over the entirety of Germany

7.  Practice plotting, and using the functions from this session on data or your own. Think about practical applications of these functions and how you may want to use it in your own work.

8.  If you're a bit more advanced than this, check out the other chapters in <https://www.paulamoraga.com/book-spatial/> or <https://rspatial.org/analysis/index.html>
...
map2 <- rnaturalearth::ne_states("Germany", returnclass = "sf") #may require additional package installation

# 1. convert both to spatvectors



# 2. subset map2 with the first 5 geometries and label it map3 



# install.packages("geodata")
library(geodata)
d <- worldclim_country(country = "Germany", var = "tmin",
                       path = tempdir())
...
# 4. Plot the average over the layers



# 5. plot average for only map3 areas



# 6.  What is the average minimum temperature, for each month over the entirety of Germany. If you get NA values, examine the function's help page to find out a way around this.
